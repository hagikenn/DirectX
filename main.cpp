...
//   namespace pmr {
//      template<class T> using vector = std::vector<T, polymorphic_allocator<T>>; // C++17から
//   }
//} // namespace std
...
void UploadTextureData(ID3D12Resource* texture, const DirectX::ScratchImage& mipImages) {
	...
	// 全MipMapについて
	for (size_t mipLevel_ = 0; mipLevel_ < metadata.mipLevels; ++mipLevel_) {
		// MipMapLevelを指定して各Imageを取得
		const DirectX::Image* img = mipImages.GetImage(mipLevel_, 0, 0);
		// Textureに転送
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	staticSamplers[0].ComparisonFunc = D3D12_COMPARISON_FUNC_NEVER;//比較する
	staticSamplers[0].MaxLOD = D3D12_FLOAT32_MAX;//ありったけのMipmapを使う
	staticSamplers[0].ShaderRegister = 0;//レジスタ番号0を使う
	staticSamplers[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;//PixeShaderで使う
	...
	graphicsPipelineStateDesc.InputLayout = inputLayoutDesc;                                                  // InputLayout                                            // InputLayout
	graphicsPipelineStateDesc.VS = { vertexShaderBlob->GetBufferPointer(), vertexShaderBlob->GetBufferSize() }; // VertexShader
	graphicsPipelineStateDesc.PS = { pixelShaderBlob->GetBufferPointer(), pixelShaderBlob->GetBufferSize() };   // PixelShader
	graphicsPipelineStateDesc.BlendState = blendDesc;                                                         // BlendState                                                   // BlendDesc
	...
	ID3D12Resource* vertexResource = CreateBufferResource(device, sizeof(VertexDate) * 3);
	// マテリアル用のリソースを作る。今回はcolor１つ分のサイズを用意する
	ID3D12Resource* materialResource = CreateBufferResource(device, sizeof(Vector4));
	// マテリアルにデータを書き込む
	VertexDate* materialData = nullptr;
	// 書き込むためのアドレスを取得
	...
	*wvpData = MakeIdentity4x4();
	...
	// 今回は赤を書き込んでみる
	//*materialData = VertexDate(1.0f, 0.0f, 0.0f, 1.0f);
	...
	vertexBufferView.SizeInBytes = sizeof(VertexDate) * 3;
	// １頂点あたりのサイズ
	...
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D; // 2Dテキスト
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			ImGui::Begin("Window");
			ImGui::ColorEdit3("color", &materialData->position.x);
			ImGui::End();
