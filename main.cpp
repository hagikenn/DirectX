...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//実際に頂点リソースを作る
	ID3D12Resource* vertexResoure = nullptr;
	hr = device->CreateCommittedResource(&uploadHeapProperties, D3D12_HEAP_FLAG_NONE,
		&vertexResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&vertexResoure));
	assert(SUCCEEDED(hr));

	//頂点バッファビューを作成する
	D3D12_VERTEX_BUFFER_VIEW vertexBufferView{};
	//リソースの先頭のアドレスから使う
	vertexBufferView.BufferLocation = vertexResoure->GetGPUVirtualAddress();
	...
	//書き込むためのアドレスを取得
	vertexResoure->Map(0, nullptr, reinterpret_cast<void**>(&vertexData));
	//左下
	...
	//クライアント領域のサイズと一緒にして画面全体にして画面全体に表示
